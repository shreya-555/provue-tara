...
## 5. Evaluation Script
I built a repeatable evaluation script (`tests/eval.ts`) using `tsx` that hits the local `/ask` endpoint. 
It covers 12 specific edge cases required by the prompt, including:
1. Single lookup & date filtering boundaries.
2. Wildcard merchant alias matching (Swiggy).
3. Ignoring self-transfers during spending calculations.
4. Handling missing/empty categories ("uncategorized").
5. Strict distinction between Market Period returns and Portfolio Realized returns.
6. The "No Data" edge case (Rent in April 2025).
