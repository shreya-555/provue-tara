...
function applyCompatLayer({
  ...
  if (mode === "jsonSchema") {
    const standardSchema = chunkZRSV37SF_cjs.toStandardSchema(schema);
    for (const compat of compatLayers) {
      if (compat.shouldApply()) {
        const compatSchema = compat.processToCompatSchema(standardSchema);
        return chunkZRSV37SF_cjs.standardSchemaToJSONSchema(compatSchema);
      }
    }
    return chunkZRSV37SF_cjs.standardSchemaToJSONSchema(standardSchema);
  } else {
    let zodSchema2;
    if (isZodType(schema)) {
      zodSchema2 = schema;
    } else {
      if (chunkZRSV37SF_cjs.isStandardSchemaWithJSON(schema)) {
...
var SchemaCompatLayer3 = class {
  ...
  processToJSONSchema(schema, io = "input") {
    const standardSchema = chunkZRSV37SF_cjs.toStandardSchema(schema);
    const jsonSchema2 = chunkZRSV37SF_cjs.standardSchemaToJSONSchema(standardSchema, {
      target: "draft-07",
      io
  ...
  processToCompatSchema(schema) {
    return {
      "~standard": {
        version: 1,
        vendor: "mastra",
        validate: (value) => chunkZRSV37SF_cjs.toStandardSchema(schema)["~standard"].validate(value),
        jsonSchema: {
          input: () => {
  ...
  toJSONSchema(zodSchema2) {
    ...
    const standardSchema = chunkZRSV37SF_cjs.toStandardSchema(zodSchema2);
    const jsonSchema2 = chunkZRSV37SF_cjs.standardSchemaToJSONSchema(standardSchema, {
      target,
      io: "input"
      // Use input mode so fields with defaults are optional
...
var OpenAISchemaCompatLayer = class extends SchemaCompatLayer3 {
  ...
  processToAISDKSchema(zodSchema2) {
    ...
    const transformedJsonSchema = chunkZRSV37SF_cjs.standardSchemaToJSONSchema(compat);
  ...
  processToCompatSchema(schema) {
    const originalStandardSchema = chunkZRSV37SF_cjs.toStandardSchema(schema);
    return {
      "~standard": {
        version: 1,
        vendor: "mastra",
...
function applyOpenAICompatTransforms(schema, modelInfo) {
  ...
  return chunkZRSV37SF_cjs.toStandardSchema(processedZodSchema);
}
