...
function isToolLoopAgentLike(obj) {
  if (!obj) return false;
  if (obj instanceof chunk4SCYX53T_cjs.ToolLoopAgent) return true;
  ...
  async handlePrepareStep(args, currentResult) {
    if (this.settings.prepareStep) {
      ...
      if (currentResult.model) {
        const resolvedModel = await chunkDYWGJ6ZD_cjs.resolveModelConfig(currentResult.model);
        if (!chunkPPP5XYB5_cjs.isSupportedLanguageModel(resolvedModel)) {
...
function toolLoopAgentToMastraAgent(agent, options) {
  ...
  const id = agentConfig.id || options?.fallbackName || `tool-loop-agent-${chunkYG4NX3QT_cjs.generateId()}`;
  return new chunkPPP5XYB5_cjs.Agent({
    ...agentConfig,
    id,
...
//# sourceMappingURL=chunk-ISV5UIZU.cjs.map
//# sourceMappingURL=chunk-ISV5UIZU.cjs.map