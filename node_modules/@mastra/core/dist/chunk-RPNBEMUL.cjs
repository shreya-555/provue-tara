...
async function executeAgent(agent, item, signal, requestContext, experimentId, versions) {
  ...
  const reqCtx = requestContext ? new chunk64VGLWFD_cjs.RequestContext(Object.entries(requestContext)) : void 0;
  const tracingOptions = experimentId ? { metadata: { experimentId } } : void 0;
  const rawResult = chunkPPP5XYB5_cjs.isSupportedLanguageModel(model) ? await agent.generate(input, {
    scorers: {},
...
function extractResumeData(item) {
  const perStep = item.resumeSteps !== void 0 ? item.resumeSteps : item.metadata?.resumeSteps;
  const flat = item.resumeData !== void 0 ? item.resumeData : item.metadata?.resumeData;
  return { perStep, flat };
}
async function executeWorkflow(workflow, item, requestContext) {
  const reqCtx = requestContext ? new chunk64VGLWFD_cjs.RequestContext(Object.entries(requestContext)) : void 0;
  const observabilityContext = chunkP5FPVDFY_cjs.resolveObservabilityContext({});
  ...
  const hasResumeData = perStep !== void 0 || flat !== void 0;
  if (hasResumeData) {
    let cycle = 0;
...
function handleWorkflowResult(result) {
  ...
  if (result.status === "suspended") {
    return {
      ...
      error: {
        message: "Workflow suspended \u2014 provide resume data via item.resumeSteps/item.resumeData (or metadata.resumeSteps/metadata.resumeData) to auto-resume"
      },
      traceId,
      spanId,
      stepResults: result.steps,
...
function toScorerTargetEntityType(targetType) {
  switch (targetType) {
    case "agent":
      return chunkXDJV2I6J_cjs.EntityType.AGENT;
    case "workflow":
      return chunkXDJV2I6J_cjs.EntityType.WORKFLOW_RUN;
    case "scorer":
      return chunkXDJV2I6J_cjs.EntityType.SCORER;
    default:
      return void 0;
  }
}
...
async function extractTrajectoryFromStorage(storage, traceId) {
  ...
  try {
    ...
    return chunkWZHQJ2FB_cjs.extractTrajectoryFromTrace(trace.spans);
  } catch {
    return void 0;
  }
}
async function runScorersForItem(scorers, item, output, storage, runId, targetType, targetId, itemId, scorerInput, scorerOutput, traceId, workflowData) {
  ...
  if (hasTrajectoryScorer) {
    ...
    trajectoryOutput = traceTrajectory ?? (scorerOutput ? chunkWZHQJ2FB_cjs.extractTrajectory(scorerOutput) : { steps: [] });
  }
  ...
  const settled = await Promise.allSettled(
    scorers.map(async (scorer) => {
      ...
      if (storage && result.score !== null) {
        try {
          await chunkQZTBGYKZ_cjs.validateAndSaveScore(storage, {
            scorerId: scorer.id,
            score: result.score,
...
async function runStepScorersForItem(stepScorers, item, workflowData, storage, runId, targetType, targetId, itemId, traceId) {
  ...
  for (const stepId of stepIds) {
    ...
    const targetCorrelationContext = {
      ...traceId ? { traceId } : {},
      entityType: chunkXDJV2I6J_cjs.EntityType.WORKFLOW_STEP,
      entityId: stepId,
      entityName: stepId,
      experimentId: runId
    };
      ...
      scorers.map(async (scorer) => {
        try {
          const scoreResult = await scorer.run({
            ...
            scoreSource: "experiment",
            targetScope: "span",
            targetEntityType: chunkXDJV2I6J_cjs.EntityType.WORKFLOW_STEP,
...
//# sourceMappingURL=chunk-RPNBEMUL.cjs.map
//# sourceMappingURL=chunk-RPNBEMUL.cjs.map