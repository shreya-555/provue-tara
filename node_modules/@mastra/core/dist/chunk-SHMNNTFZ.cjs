...
var chunkP5FPVDFY_cjs = require('./chunk-P5FPVDFY.cjs');
var chunkEDFBQDWJ_cjs = require('./chunk-EDFBQDWJ.cjs');
var chunkXDJV2I6J_cjs = require('./chunk-XDJV2I6J.cjs');
var chunkRUTNLYPF_cjs = require('./chunk-RUTNLYPF.cjs');
...
function validateCron(cron, timezone) {
  const job = new croner.Cron(cron, { timezone });
  job.nextRun();
}
function computeNextFireAt(cron, options) {
  const job = new croner.Cron(cron, { timezone: options?.timezone });
...
function createTripWireFromChunk(chunk) {
  const { payload } = chunk;
  return new chunkPPP5XYB5_cjs.TripWire(
...
var StepExecutor = class extends chunkOXCC7WGI_cjs.MastraBase {
  mastra;
  constructor({ mastra }) {
    super({ name: "StepExecutor", component: chunk4CVPHNP6_cjs.RegisteredLogger.WORKFLOW });
    this.mastra = mastra;
  }
  __registerMastra(mastra) {
    this.mastra = mastra;
  ...
  async execute(params) {
    ...
    const { inputData, validationError } = await chunkPPP5XYB5_cjs.validateStepInput({
    ...
    try {
      ...
      const stepOutput = await chunkEDFBQDWJ_cjs.executeWithContext({
        span: params.tracingContext?.currentSpan,
        fn: () => step.execute(
          chunkPPP5XYB5_cjs.createDeprecationProxy(
            {
              ...
              getStepResult: chunkPPP5XYB5_cjs.getStepResult.bind(this, stepResults),
              suspend: async (suspendPayload, suspendOptions) => {
                const { suspendData, validationError: validationError2 } = await chunkPPP5XYB5_cjs.validateStepSuspendData({
              ...
              writer: new chunkRUTNLYPF_cjs.ToolStream(
                {
              ...
              ...chunkP5FPVDFY_cjs.createObservabilityContext(params.tracingContext)
            },
            {
              paramName: "runCount",
              deprecationMessage: chunkPPP5XYB5_cjs.runCountDeprecationMessage,
              logger: this.logger
            }
          )
        )
      ...
      const endedAt = Date.now();
      const errorInstance = chunkXSOONORA_cjs.getErrorFromUnknown(error, {
      ...
      const mastraError = new chunkXSOONORA_cjs.MastraError(
        {
          ...
          domain: chunkXSOONORA_cjs.ErrorDomain.MASTRA_WORKFLOW,
      ...
      return {
        ...
        tripwire: error instanceof chunkPPP5XYB5_cjs.TripWire ? {
      ...
      chunkPPP5XYB5_cjs.createDeprecationProxy(
        {
          ...
          abortSignal: abortController?.signal,
          // TODO
          ...chunkP5FPVDFY_cjs.createObservabilityContext(),
          iterationCount
        },
        {
          paramName: "runCount",
...
async function processWorkflowLoop({
  ...
  const reqContext = new chunk64VGLWFD_cjs.RequestContext(Object.entries(requestContext ?? {}));
...
var WorkflowEventProcessor = class extends EventProcessor {
  ...
  async errorWorkflow({
    ...
    await this.mastra.pubsub.publish("workflows", {
      ...
      data: {
        ...
        resumeSteps,
        stepResults,
        prevResult: { status: "failed", error: chunkXSOONORA_cjs.getErrorFromUnknown(e).toJSON() },
        requestContext,
        resumeData,
        activeStepsPath: {},
        parentWorkflow
  ...
  async processWorkflowSuspend(args) {
    ...
    if (parentWorkflow) {
      ...
      const suspendedStepId = workflow && executionPath ? getStep(workflow, executionPath)?.id : void 0;
  ...
  async processWorkflowStepRun({
    ...
    if (!executionPath?.length) {
      return this.errorWorkflow(
        ...
        new chunkXSOONORA_cjs.MastraError({
          id: "MASTRA_WORKFLOW",
    ...
    if ((step.type === "parallel" || step.type === "conditional") && executionPath.length > 1 && (!restart || restart && restart.isParallelOrConditionalRestarted)) {
    ...
    if (step.step instanceof EventedWorkflow || step.step.component === "WORKFLOW") {
      if (resumeSteps?.length === 1 && resumeSteps[0] === step.step.id) {
        ...
        const timeTravelParams = chunkPPP5XYB5_cjs.createTimeTravelExecutionParams({
        ...
        const nestedWorkflow = step.step;
        const restartParams = chunkPPP5XYB5_cjs.createRestartExecutionParams({ snapshot, graph: nestedWorkflow.buildExecutionGraph() });
    ...
    const rc = new chunk64VGLWFD_cjs.RequestContext();
    for (const [key, value] of Object.entries(requestContext)) {
      rc.set(key, value);
    }
    const { resumeData: timeTravelResumeData, validationError: timeTravelResumeValidationError } = await chunkPPP5XYB5_cjs.validateStepResumeData({
    ...
    if (abortController?.signal?.aborted) {
      ...
      return this.mastra.pubsub.publish("workflows", {
        ...
        data: {
          ...
          prevResult: { ...stepResult, status: "canceled" },
          //set the status to canceled to indicate the workflow was canceled
          activeStepsPath,
          requestContext,
          perStep,
          state: updatedState,
  ...
  async aggregateBranchResults({
    ...
    branchEntry.steps.forEach((branch, idx) => {
      ...
      const res = stepResults?.[branch.step.id];
      if (!res || !res.status) {
        return;
      }
  ...
  async #dispatch(event) {
    ...
    if (!workflow) {
      if (type === "workflow.fail" || type === "workflow.end" || type === "workflow.cancel") ; else {
        return this.errorWorkflow(
          workflowData,
          new chunkXSOONORA_cjs.MastraError({
            id: "MASTRA_WORKFLOW",
...
function cloneWorkflow(workflow, opts) {
  const wf = new chunkPPP5XYB5_cjs.Workflow({
    id: opts.id,
...
function isToolStep(input) {
  return input instanceof chunk7Y2NL3JH_cjs.Tool;
}
function isStepParams(input) {
  return input !== null && typeof input === "object" && "id" in input && "execute" in input && !(input instanceof chunkPPP5XYB5_cjs.Agent) && !(input instanceof chunk7Y2NL3JH_cjs.Tool);
}
function isProcessor(obj) {
  return obj !== null && typeof obj === "object" && "id" in obj && typeof obj.id === "string" && !(obj instanceof chunkPPP5XYB5_cjs.Agent) && !(obj instanceof chunk7Y2NL3JH_cjs.Tool) && (typeof obj.processInput === "function" || typeof obj.processInputStep === "function" || typeof obj.processOutputStream === "function" || typeof obj.processOutputResult === "function" || typeof obj.processOutputStep === "function");
}
function areProcessorMessageArraysEqual(before, after) {
  ...
  return before.length === after.length && before.every((message, index) => chunkDCCAHUCG_cjs.messagesAreEqual(message, after[index]));
}
function createStep(params, agentOrToolOptions) {
  if (chunkPPP5XYB5_cjs.isAgentCompatible(params)) {
...
async function processAgentStream(params) {
  ...
  for await (const chunk of fullStream) {
    ...
    if (streamFormat !== "legacy") {
      await chunkPPP5XYB5_cjs.forwardAgentStreamChunk({ writer, chunk });
    }
  }
  try {
    await pubsub.publish(`workflow.events.v2.${runId}`, {
...
function createStepFromAgent(params, agentOrToolOptions) {
  ...
  return {
    ...
    execute: async ({
      ...
      const observabilityContext = chunkP5FPVDFY_cjs.resolveObservabilityContext(obsFields);
      const logger = mastra?.getLogger();
      const toolData = {
        name: params.name ?? params.id,
        args: inputData
      };
      const isV2Model = chunkPPP5XYB5_cjs.isSupportedLanguageModel(await params.getModel({ requestContext }));
...
function createStepFromProcessor(processor) {
  const getProcessorEntityType = (phase) => {
    switch (phase) {
      case "input":
        return chunkXDJV2I6J_cjs.EntityType.INPUT_PROCESSOR;
      case "inputStep":
        return chunkXDJV2I6J_cjs.EntityType.INPUT_STEP_PROCESSOR;
      case "outputStream":
      case "outputResult":
        return chunkXDJV2I6J_cjs.EntityType.OUTPUT_PROCESSOR;
      case "outputStep":
        return chunkXDJV2I6J_cjs.EntityType.OUTPUT_STEP_PROCESSOR;
      default:
        return chunkXDJV2I6J_cjs.EntityType.OUTPUT_PROCESSOR;
    }
  };
  ...
  return {
    ...
    inputSchema: chunkXB4FLS7A_cjs.toStandardSchema(chunkPPP5XYB5_cjs.ProcessorStepSchema),
    outputSchema: chunkXB4FLS7A_cjs.toStandardSchema(chunkPPP5XYB5_cjs.ProcessorStepOutputSchema),
    execute: async ({ inputData, requestContext, outputWriter, ...obsFields }) => {
      ...
      const abort = (reason, options) => {
        throw new chunkPPP5XYB5_cjs.TripWire(reason || `Tripwire triggered by ${processor.id}`, options, processor.id);
      };
      ...
      const buildProcessorSpanInput = () => {
        switch (phase) {
          ...
          case "inputStep": {
            const summarizedModel = chunkPPP5XYB5_cjs.summarizeProcessorModelForSpan(model);
            const summarizedTools = chunkPPP5XYB5_cjs.summarizeProcessorToolsForSpan(tools);
            const summarizedToolChoice = chunkPPP5XYB5_cjs.summarizeToolChoiceForSpan(toolChoice, tools);
            const summarizedActiveTools = chunkPPP5XYB5_cjs.summarizeActiveToolsForSpan(activeTools, tools);
            return {
              messages: messages ?? [],
              ...systemMessages ? { systemMessages } : {},
          ...
          case "outputResult": {
            const summarizedResult = chunkPPP5XYB5_cjs.summarizeProcessorResultForSpan(outputResult ?? defaultOutputResult);
            return {
              messages: messages ?? [],
        ...
        switch (phase) {
          ...
          case "inputStep": {
            ...
            if (payload.model !== void 0 && payload.model !== model) {
              const summarizedModel = chunkPPP5XYB5_cjs.summarizeProcessorModelForSpan(payload.model);
              if (summarizedModel) {
                output.model = summarizedModel;
              }
            }
            if (payload.tools !== void 0 && payload.tools !== tools) {
              const summarizedTools = chunkPPP5XYB5_cjs.summarizeProcessorToolsForSpan(payload.tools);
              if (summarizedTools) {
                output.tools = summarizedTools;
              }
            }
            if (payload.toolChoice !== void 0 && payload.toolChoice !== toolChoice) {
              const summarizedToolChoice = chunkPPP5XYB5_cjs.summarizeToolChoiceForSpan(payload.toolChoice, payload.tools ?? tools);
            ...
            if (payload.activeTools !== void 0 && payload.activeTools !== activeTools) {
              const summarizedActiveTools = chunkPPP5XYB5_cjs.summarizeActiveToolsForSpan(payload.activeTools, payload.tools ?? tools);
      ...
      const processorObservabilityContext = chunkP5FPVDFY_cjs.createObservabilityContext(
      ...
      if (processorStates) {
        ...
        if (!ps) {
          ps = new chunkPPP5XYB5_cjs.ProcessorState();
      ...
      const processorMessageList = messageList ?? (Array.isArray(messages) ? new chunkDCCAHUCG_cjs.MessageList().add(messages, "input").addSystem(systemMessages ?? []) : void 0);
      const baseContext = {
        abort,
        ...
        ...processorMessageList ? {
          sendSignal: chunkPPP5XYB5_cjs.createProcessorSendSignal({
      ...
      const executePhaseWithSpan = async (fn) => {
        try {
          const result = await chunkEDFBQDWJ_cjs.executeWithContext({ span: processorSpan, fn });
          processorSpan?.end({ output: buildProcessorSpanOutput(result) });
          return result;
        } catch (error) {
          if (error instanceof chunkPPP5XYB5_cjs.TripWire) {
        ...
        switch (phase) {
          case "input": {
            if (processor.processInput) {
              ...
              if (result instanceof chunkDCCAHUCG_cjs.MessageList) {
                ...
                chunkPPP5XYB5_cjs.ProcessorRunner.applyMessagesToMessageList(
                  result,
          ...
          case "inputStep": {
            if (processor.processInputStep) {
              ...
              const validatedResult = await chunkPPP5XYB5_cjs.ProcessorRunner.validateAndFormatProcessInputStepResult(result, {
                messageList: passThrough.messageList,
                processor,
                stepNumber: stepNumber ?? 0
              });
            ...
            if (processor.processOutputStream && part) {
              ...
              if (!processorSpan2 && parentSpan) {
                processorSpan2 = parentSpan.createChildSpan({
                  ...
                  entityType: chunkXDJV2I6J_cjs.EntityType.OUTPUT_PROCESSOR,
                  entityId: processor.id,
                  entityName: processor.name ?? processor.id,
                  attributes: {
                    processorExecutor: "workflow",
              ...
              const processorObservabilityContext2 = chunkP5FPVDFY_cjs.createObservabilityContext(
...
chunkPPP5XYB5_cjs.__registerEventedCreateWorkflow(createWorkflow);
var EventedWorkflow = class extends chunkPPP5XYB5_cjs.Workflow {
  #schedules;
  constructor(params) {
    super(params);
  ...
  async createRun(options) {
    ...
    if (workflowsStore && !supportsConcurrentUpdates) {
      throw new chunkXSOONORA_cjs.MastraError({
        ...
        domain: chunkXSOONORA_cjs.ErrorDomain.MASTRA,
        category: chunkXSOONORA_cjs.ErrorCategory.USER,
...
var EventedRun = class extends chunkPPP5XYB5_cjs.Run {
  constructor(params) {
    super(params);
  ...
  async start({
    ...
    requestContext = requestContext ?? new chunk64VGLWFD_cjs.RequestContext();
    const workflowsStore = await this.mastra?.getStorage()?.getStore("workflows");
    await workflowsStore?.persistWorkflowSnapshot({
  ...
  stream({
    ...
    this.streamOutput = new chunkPPP5XYB5_cjs.WorkflowRunOutput({
      runId: this.runId,
      workflowId: this.workflowId,
      stream
    });
  ...
  async resume(params) {
    ...
    const requestContext = new chunk64VGLWFD_cjs.RequestContext();
...
function getNestedWorkflow(mastra, { workflowId, executionPath, parentWorkflow }) {
  ...
  workflow = workflow ?? (mastra.__hasInternalWorkflow(workflowId) ? mastra.__getInternalWorkflow(workflowId) : mastra.getWorkflow(workflowId));
...
var EventedExecutionEngine = class extends chunkPPP5XYB5_cjs.ExecutionEngine {
  eventProcessor;
  constructor({
    mastra,
    eventProcessor,
  ...
  async execute(params) {
    ...
    const finishCb = async (event, ack) => {
      ...
      if (["workflow.end", "workflow.fail", "workflow.suspend"].includes(event.type)) {
        ...
        if (event.type === "workflow.fail" && event.data.stepResults) {
          event.data.stepResults = chunkPPP5XYB5_cjs.hydrateSerializedStepErrors(event.data.stepResults);
        }
    ...
    for (const [stepId, stepResult] of Object.entries(stepResultsWithoutTopLevelState)) {
      ...
      cleanStepResults[stepId] = chunkPPP5XYB5_cjs.cleanStepResult(stepResult);
    }
    let callbackArg;
...
exports.computeNextFireAt = computeNextFireAt;
exports.createStep = createStep;
exports.createWorkflow = createWorkflow;
exports.getStep = getStep;
exports.validateCron = validateCron;
//# sourceMappingURL=chunk-SHMNNTFZ.cjs.map
//# sourceMappingURL=chunk-SHMNNTFZ.cjs.map