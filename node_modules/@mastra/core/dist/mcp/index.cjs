...
var MCPServerBase = class extends chunkOXCC7WGI_cjs.MastraBase {
    ...
    if (this.originalTools && typeof this.originalTools === "object") {
      Object.entries(this.originalTools).forEach(([key, tool]) => {
        try {
          ...
          if (!(error instanceof chunkXSOONORA_cjs.MastraError) || error.id !== "MASTRA_ADD_TOOL_DUPLICATE_KEY") {
            throw error;
          }
        }
      });
    ...
    if (this.agents && typeof this.agents === "object") {
      Object.entries(this.agents).forEach(([key, agent]) => {
        try {
          ...
          if (!(error instanceof chunkXSOONORA_cjs.MastraError) || error.id !== "MASTRA_ADD_AGENT_DUPLICATE_KEY") {
            throw error;
          }
        }
      });
    ...
    if (this.workflows && typeof this.workflows === "object") {
      Object.entries(this.workflows).forEach(([key, workflow]) => {
        try {
          ...
          if (!(error instanceof chunkXSOONORA_cjs.MastraError) || error.id !== "MASTRA_ADD_WORKFLOW_DUPLICATE_KEY") {
            throw error;
          }
        }
      });
   ...
   */
  constructor(config) {
    super({ component: chunk4CVPHNP6_cjs.RegisteredLogger.MCP_SERVER, name: config.name });
