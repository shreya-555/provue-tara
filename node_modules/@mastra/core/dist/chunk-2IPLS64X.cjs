...
var chunkPXMHGMJT_cjs = require('./chunk-PXMHGMJT.cjs');
var chunkSHMNNTFZ_cjs = require('./chunk-SHMNNTFZ.cjs');
var chunkISV5UIZU_cjs = require('./chunk-ISV5UIZU.cjs');
var chunkHSHFTZDP_cjs = require('./chunk-HSHFTZDP.cjs');
var chunkRPNBEMUL_cjs = require('./chunk-RPNBEMUL.cjs');
var chunkQZTBGYKZ_cjs = require('./chunk-QZTBGYKZ.cjs');
var chunkPPP5XYB5_cjs = require('./chunk-PPP5XYB5.cjs');
var chunkDYWGJ6ZD_cjs = require('./chunk-DYWGJ6ZD.cjs');
var chunkQ5I3BBUU_cjs = require('./chunk-Q5I3BBUU.cjs');
...
function createUndefinedPrimitiveError(type, value, key) {
  ...
  return new chunkXSOONORA_cjs.MastraError({
    id: errorId,
...
var Mastra = class {
  ...
  get agentThreadStreamRuntime() {
    return chunkPPP5XYB5_cjs.agentThreadStreamRuntime;
  }
  get workers() {
    return this.#workers;
  }
  ...
  get datasets() {
    if (!this.#datasets) {
      this.#datasets = new chunkRPNBEMUL_cjs.DatasetsManager(this);
    }
  ...
  generateId(context) {
    if (this.#idGenerator) {
      ...
      if (!id) {
        const error = new chunkXSOONORA_cjs.MastraError({
  ...
  registerExporter(exporter, instance, entrypoint) {
    if (this.#observability instanceof chunkP5FPVDFY_cjs.NoOpObservability) {
   ...
   */
  constructor(config) {
    chunkHSHFTZDP_cjs.initContextStorage();
    this.#serverCache = config?.cache ?? new chunkZLNWRIBN_cjs.InMemoryServerCache();
    this.#editor = config?.editor;
    this.#versions = config?.versions;
    this.#environment = config?.environment ?? process.env.NODE_ENV;
    this.#toolPayloadTransform = chunkVPZGHUNF_cjs.normalizeToolPayloadTransformPolicy(
      config?.transform ?? config?.toolPayloadProjection
    );
    if (config?.pubsub) {
      this.#pubsub = config.pubsub;
    } else {
      this.#pubsub = new chunkCVF4W47C_cjs.EventEmitterPubSub();
    }
    this.#events = {};
    ...
    if (workersOption === false) ; else if (Array.isArray(workersOption)) {
      ...
      if (pubsubModes.includes("pull")) {
        defaultWorkers.push(new chunkPXMHGMJT_cjs.OrchestrationWorker());
      }
      if (config?.backgroundTasks?.enabled) {
        defaultWorkers.push(new chunkPXMHGMJT_cjs.BackgroundTaskWorker(config.backgroundTasks));
      }
    ...
    if (config?.logger === false) {
      logger = chunkQ5I3BBUU_cjs.noopLogger;
    } else {
      if (config?.logger) {
        logger = config.logger;
      } else {
        const levelOnEnv = process.env.NODE_ENV === "production" && process.env.MASTRA_DEV !== "true" ? chunk4CVPHNP6_cjs.LogLevel.WARN : chunk4CVPHNP6_cjs.LogLevel.INFO;
        logger = new chunk4CVPHNP6_cjs.ConsoleLogger({ name: "Mastra", level: levelOnEnv });
      }
    }
    this.#logger = logger;
    ...
    if (storage) {
      storage = chunkPPP5XYB5_cjs.augmentWithInit(storage);
    }
    if (config?.observability) {
      if (typeof config.observability.getDefaultInstance === "function") {
        ...
        this.#observability = new chunkP5FPVDFY_cjs.NoOpObservability();
      }
    } else {
      this.#observability = new chunkP5FPVDFY_cjs.NoOpObservability();
    }
    const dualLogger = new chunkQ5I3BBUU_cjs.DualLogger(this.#logger, () => this.loggerVNext);
    ...
    for (const gateway of chunkDYWGJ6ZD_cjs.defaultGateways) {
    ...
    chunkMR7ZWBL6_cjs.registerHook("onScorerRun" /* ON_SCORER_RUN */, chunkQZTBGYKZ_cjs.createOnScorerHook(this));
    this.#observability.setMastraContext({ mastra: this });
    ...
    const bgManager = new chunkIHOYML2N_cjs.BackgroundTaskManager(this.#backgroundTaskConfig);
      ...
      try {
        ...
        if (!existing) {
          await schedulesStore.createSchedule({
            ...
            status: "active",
            nextFireAt: chunkSHMNNTFZ_cjs.computeNextFireAt(cfg.cron, { timezone: cfg.timezone, after: now }),
            createdAt: now,
            updatedAt: now,
        ...
        if (cronChanged || timezoneChanged) {
          patch.nextFireAt = chunkSHMNNTFZ_cjs.computeNextFireAt(cfg.cron, { timezone: cfg.timezone, after: now });
        }
  ...
  getChannels() {
    ...
    for (const [agentKey, agent] of Object.entries(this.#agents ?? {})) {
      ...
      if (agentChannels instanceof chunkPPP5XYB5_cjs.AgentChannels) {
  ...
  addAgent(agent, key, options) {
    ...
    if (chunkPPP5XYB5_cjs.isDurableAgentLike(agent)) {
    ...
    let mastraAgent;
    if (chunkISV5UIZU_cjs.isToolLoopAgentLike(agent)) {
      mastraAgent = chunkISV5UIZU_cjs.toolLoopAgentToMastraAgent(agent, { fallbackName: key });
    } else {
      mastraAgent = agent;
    }
   ...
   */
  setStorage(storage) {
    this.#storage = chunkPPP5XYB5_cjs.augmentWithInit(storage);
    this.#storage?.__registerMastra?.(this);
    this.#ensureBackgroundTaskManager();
  }
  setLogger({ logger }) {
    const dualLogger = new chunkQ5I3BBUU_cjs.DualLogger(logger, () => this.loggerVNext);
    this.#logger = dualLogger;
    if (this.#agents) {
   ...
   */
  get loggerVNext() {
    return this.#observability.getDefaultInstance()?.getLoggerContext?.() ?? chunkP5FPVDFY_cjs.noOpLoggerContext;
  }
  /**
   ...
   */
  get metrics() {
    return this.#observability.getDefaultInstance()?.getMetricsContext?.() ?? chunkP5FPVDFY_cjs.noOpMetricsContext;
  }
  getServerMiddleware() {
  ...
  async handleWorkflowEvent(event) {
    if (!this.#workflowEventProcessor) {
      this.#workflowEventProcessor = new chunkSHMNNTFZ_cjs.WorkflowEventProcessor({ mastra: this });
    }
  ...
  async startWorkers(name) {
    if (!name && this.#shouldEnableScheduler() && this.#storage && !this.#findSchedulerWorker()) {
      const sw = new chunkPXMHGMJT_cjs.SchedulerWorker(this.#schedulerConfig);
      ...
      if (pushOnly && !this.#pushSubscription) {
        const cb = (event, ack) => {
          void this.handleWorkflowEvent(event).then((result) => {
            if (result.ok && ack) {
              return ack().catch(
                (err) => this.#logger?.error?.("Error acking workflow event in push subscription", err)
              );
            }
  ...
  #syncGatewayRegistry() {
    try {
      ...
      import('./provider-registry-TI2S24TW.cjs').then(async ({ GatewayRegistry }) => {
...
//# sourceMappingURL=chunk-2IPLS64X.cjs.map
//# sourceMappingURL=chunk-2IPLS64X.cjs.map