...
function schemaToTs(schema, io) {
  if (!chunkXB4FLS7A_cjs.isStandardSchemaWithJSON(schema)) return "unknown";
  try {
    const json = chunkXB4FLS7A_cjs.standardSchemaToJSONSchema(schema, { io });
...
var StdioCodeModeTransport = class {
  async run(opts) {
    ...
    const dir = await promises.mkdtemp(path.join(os.tmpdir(), "mastra-code-mode-"));
    const suffix = crypto.randomBytes(4).toString("hex");
    const programPath = path.join(dir, `program-${suffix}.ts`);
    await promises.writeFile(programPath, buildProgramModule(program), "utf8");
    const runnerSource = buildRunner({ programModule: url.pathToFileURL(programPath).href, externals });
    const runnerPath = path.join(dir, `runner-${suffix}.mjs`);
    await promises.writeFile(runnerPath, runnerSource, "utf8");
    const logs = [];
    let done;
    let stdoutBuffer = "";
    let resolveDone;
...
var codeModeInputSchema = v4.z.object({
  code: v4.z.string().describe(
...
var codeModeOutputSchema = v4.z.object({
  success: v4.z.boolean(),
  result: v4.z.unknown().optional(),
  logs: v4.z.array(v4.z.string()).optional(),
  error: v4.z.object({
    message: v4.z.string(),
    name: v4.z.string().optional(),
    line: v4.z.number().optional()
  }).optional()
});
  ...
  return chunk7Y2NL3JH_cjs.createTool({
    ...
    execute: async ({ code }, ctx) => {
      ...
      const dispatch = async (toolId, args) => {
        ...
        if (chunk7Y2NL3JH_cjs.isValidationError(result)) {
...
exports.StdioCodeModeTransport = StdioCodeModeTransport;
...
exports.createCodeMode = createCodeMode;
exports.createCodeModeInstructions = createCodeModeInstructions;
exports.createCodeModeTool = createCodeModeTool;
exports.generateStubs = generateStubs;
exports.jsonSchemaToTsString = jsonSchemaToTsString;
//# sourceMappingURL=chunk-XEHGVQJW.cjs.map
//# sourceMappingURL=chunk-XEHGVQJW.cjs.map