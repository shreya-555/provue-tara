...
var chunkDYWGJ6ZD_cjs = require('../../chunk-DYWGJ6ZD.cjs');
...
var chunkIHOYML2N_cjs = require('../../chunk-IHOYML2N.cjs');
...
async function resolveScope(agent, mergedOptions) {
  const requestContext = mergedOptions?.requestContext ?? new chunk64VGLWFD_cjs.RequestContext();
  const memory = await agent.getMemory();
  if (!memory) return null;
  const threadIdFromContext = requestContext.get(chunk64VGLWFD_cjs.MASTRA_THREAD_ID_KEY);
  const resourceIdFromContext = requestContext.get(chunk64VGLWFD_cjs.MASTRA_RESOURCE_ID_KEY);
...
async function runDurableStreamUntilIdle(agent, messages, streamOptions, deps) {
  ...
  const mergedOptions = chunkNLACGSDP_cjs.deepMerge(
    defaultOptions,
    restStreamOptions ?? {}
  );
...
async function prepareForDurableExecution(options) {
  ...
  const requestContext = providedRequestContext ?? new chunk64VGLWFD_cjs.RequestContext();
  const requestVersions = requestContext.get(chunk64VGLWFD_cjs.MASTRA_VERSIONS_KEY);
  let mergedVersions = chunk64VGLWFD_cjs.mergeVersionOverrides(mastra?.getVersionOverrides?.(), requestVersions);
  if (execOptions?.versions) {
    mergedVersions = chunk64VGLWFD_cjs.mergeVersionOverrides(mergedVersions, execOptions.versions);
  }
  if (mergedVersions) {
    requestContext.set(chunk64VGLWFD_cjs.MASTRA_VERSIONS_KEY, mergedVersions);
  }
  ...
  const messageList = new chunkDCCAHUCG_cjs.MessageList({
    threadId,
    resourceId
  });
  ...
  if (inputProcessors.length > 0) {
    try {
      ...
      const { ProcessorRunner: ProcessorRunner2 } = await import('../../runner-PMEOS5WA.cjs');
  ...
  const saveQueueManager = memory ? new chunkPPP5XYB5_cjs.SaveQueueManager({
    logger,
    memory
  }) : void 0;
...
function createDurableAgentStream(options) {
    ...
    try {
      switch (streamEvent.type) {
        case AgentStreamEventTypes.CHUNK: {
          ...
          chunkPPP5XYB5_cjs.safeEnqueue(controller, chunk);
          await onChunk?.(chunk);
          break;
        }
        ...
        case AgentStreamEventTypes.FINISH: {
          ...
          chunkPPP5XYB5_cjs.safeEnqueue(controller, finishChunk);
          chunkPPP5XYB5_cjs.safeClose(controller);
          try {
            await onFinish?.(data);
          } catch (callbackError) {
  ...
  const output = new chunkPPP5XYB5_cjs.MastraModelOutput({
    model,
    stream,
    messageList,
    messageId,
...
function createDurableBackgroundTaskCheckStep() {
  return chunkPPP5XYB5_cjs.createStep({
    id: BG_CHECK_STEP_ID,
...
async function resolveRuntimeDependencies(options) {
  ...
  if (globalEntry) {
    ...
    try {
      ...
      const resolveRequestContext = new chunk64VGLWFD_cjs.RequestContext();
  ...
  if (memory) {
    saveQueueManager = new chunkPPP5XYB5_cjs.SaveQueueManager({
...
async function toolRequiresApproval(tool, globalRequireApproval, args) {
  ...
  const needsApprovalFn = chunk7Y2NL3JH_cjs.getNeedsApprovalFn(tool);
  if (needsApprovalFn) {
    try {
...
async function resolveModelFromConfig(config, mastra) {
  ...
  if (typeof modelConfigString === "string") {
    return await chunkDYWGJ6ZD_cjs.resolveModelConfig(modelConfigString, requestContext, mastra);
  }
  return await chunkDYWGJ6ZD_cjs.resolveModelConfig(
    modelConfigString,
    requestContext,
    mastra
  );
      ...
      for (let modelIndex = 0; modelIndex < modelList.length; modelIndex++) {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
          try {
            ...
            if (!chunkPPP5XYB5_cjs.isSupportedLanguageModel(model)) {
            ...
            let currentProviderOptions = chunkPPP5XYB5_cjs.mergeProviderOptions(
            ...
            if (registryEntry?.inputProcessors?.length) {
              ...
              const runner = new chunkPPP5XYB5_cjs.ProcessorRunner({
            ...
            modelSpanTracker?.setInferenceContext?.({
              ...
              providerOptions: currentProviderOptions,
              availableTools: chunkEDFBQDWJ_cjs.getStepAvailableToolNames(
                currentTools,
                currentActiveTools
              ),
              toolChoice: currentToolChoice,
            ...
            const modelResult = chunkPPP5XYB5_cjs.execute({
              runId,
              model: currentModel,
              providerOptions: currentProviderOptions,
              inputMessages,
            ...
            const outputStream = new chunkPPP5XYB5_cjs.MastraModelOutput({
              model: {
            ...
            if (registryEntry?.errorProcessors?.length) {
              try {
                ...
                const currentMessageList = new chunkDCCAHUCG_cjs.MessageList();
  ...
  return chunkPPP5XYB5_cjs.createStep({
    ...
    execute: async (params) => {
      ...
      if (bgManager && !bgConfig?.disabled && typeof cleanedArgs === "object" && cleanedArgs !== null) {
        const bgResolved = chunkIHOYML2N_cjs.resolveBackgroundConfig({
          llmBgOverrides,
          toolName,
        ...
        if (bgResolved.runInBackground) {
          try {
            const bgTask = chunkIHOYML2N_cjs.createBackgroundTask(bgManager, {
              toolName,
              toolCallId,
              args: cleanedArgs,
...
function createDurableAgenticWorkflow(options) {
  ...
  const singleIterationWorkflow = chunkPPP5XYB5_cjs.createWorkflow({
  ...
  return chunkPPP5XYB5_cjs.createWorkflow({
    ...
    async (params) => {
      ...
      if (registryEntry?.outputProcessors?.length) {
        try {
          ...
          const outputMessageList = new chunkDCCAHUCG_cjs.MessageList();
          outputMessageList.deserialize(state.messageListState);
          await runner.runOutputProcessors(outputMessageList, {}, requestContext ?? new chunk64VGLWFD_cjs.RequestContext(), 0);
        } catch (error) {
      ...
      if (registryEntry?.saveQueueManager && registryEntry.memory && durableState?.threadId && durableState?.resourceId && !durableState.observationalMemory) {
        try {
          const memoryMessageList = new chunkDCCAHUCG_cjs.MessageList();
    ...
    async (params) => {
      ...
      const logger = mastra?.getLogger?.();
      const inputMessageList = new chunkDCCAHUCG_cjs.MessageList();
      ...
      const resolveContext = requestContext ?? new chunk64VGLWFD_cjs.RequestContext();
      for (const [scorerKey, scorerEntry] of Object.entries(scorers)) {
        ...
        try {
          ...
          chunkPPP5XYB5_cjs.runScorer({
            ...
            threadId: initData.state?.threadId,
            resourceId: initData.state?.resourceId,
            ...chunkP5FPVDFY_cjs.createObservabilityContext(tracingContext)
          });
        } catch (error) {
...
var DurableAgent = class extends chunkPPP5XYB5_cjs.Agent {
  /** The wrapped agent */
  #wrappedAgent;
  ...
  constructor(config) {
    ...
    this.#innerPubsub = pubsub ?? new chunkCVF4W47C_cjs.EventEmitterPubSub();
   ...
   */
    ...
    if (this.#cacheConfig === false) {
      ...
      const resolvedCache = this.#cacheConfig ?? this.#mastra?.serverCache ?? new chunkZLNWRIBN_cjs.InMemoryServerCache();
      this.#resolvedCache = resolvedCache;
      this.#cachingPubsub = new chunkVEYVZLLD_cjs.CachingPubSub(this.#innerPubsub, resolvedCache);
    }
  }
  // ===========================================================================
