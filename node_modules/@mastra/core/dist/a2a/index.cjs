...
function messagesToPrompt(messages, options) {
  ...
  const converted = chunkDCCAHUCG_cjs.convertMessages(messages).to("AIV5.Model");
  const lines = [];
...
function createResponseMessages(text, memoryInfo = {}) {
  ...
  return new chunkDCCAHUCG_cjs.MessageList(memoryInfo).add(
    {
      role: "assistant",
      content: text
    },
...
var A2AAgent = class {
  ...
  async getMemory({ requestContext = new chunk64VGLWFD_cjs.RequestContext() } = {}) {
    if (!this.#memory) {
      return void 0;
    }
  ...
  async #consumeA2AStream({
    ...
    const messageList = new chunkDCCAHUCG_cjs.MessageList({ threadId, resourceId });
