...
function splitRuntimeModelString(input) {
  const providers = chunkXKOQ7YF6_cjs.getRegisteredProviders().sort((a, b) => b.length - a.length);
  ...
  const parsed = chunkXKOQ7YF6_cjs.parseModelString(input);
...
function isModelAllowed(allowed, candidate) {
  ...
  const activeEntries = allowed.filter((entry) => {
    ...
    return chunkXKOQ7YF6_cjs.isProviderRegistered(entry.provider);
  });
