...
var chunkR5BRW53L_cjs = require('./chunk-R5BRW53L.cjs');
...
function captureLicenseCheck({
  ...
  const license = chunkR5BRW53L_cjs.getSafeLicenseSummary();
  try {
    const ip = getRequestIp(request);
    chunkR5BRW53L_cjs.captureEEEvent("ee_license_check", user?.id || license.anonymousId || chunkR5BRW53L_cjs.getEETelemetryFallbackDistinctId(), {
...
async function buildCapabilities(auth, request, options) {
  ...
  const hasLicense = chunkR5BRW53L_cjs.isLicenseValid();
  const isCloud = isMastraCloudAuth(auth);
  const isSimple = isSimpleAuth(auth);
  const isDev = chunkR5BRW53L_cjs.isDevEnvironment();
  ...
  if (hasRBAC && rbacProvider) {
    try {
      ...
      try {
        const ip = getRequestIp(request);
        chunkR5BRW53L_cjs.captureEEEvent("ee_feature_used", user.id || license.anonymousId || chunkR5BRW53L_cjs.getEETelemetryFallbackDistinctId(), {
          feature: "rbac",
          user_id: user.id,
...
//# sourceMappingURL=chunk-IV2LFMTD.cjs.map
//# sourceMappingURL=chunk-IV2LFMTD.cjs.map