...
var chunkIV2LFMTD_cjs = require('./chunk-IV2LFMTD.cjs');
...
var CoreToolBuilder = class extends chunkOXCC7WGI_cjs.MastraBase {
  originalTool;
  options;
  logType;
  constructor(input) {
    ...
    if (!chunk7Y2NL3JH_cjs.isVercelTool(this.originalTool) && !chunk7Y2NL3JH_cjs.isProviderDefinedTool(this.originalTool)) {
        ...
        if (chunkCMKSC37Z_cjs.isZodObject(schema) && isZodV4Schema(schema)) {
          ...
          if (isBackgroundEligible) {
            nextSchema = chunkCMKSC37Z_cjs.safeExtendZodObject(nextSchema, {
              _background: chunkIHOYML2N_cjs.backgroundOverrideZodSchema
            });
          }
          if (isResumableTool) {
            nextSchema = chunkCMKSC37Z_cjs.safeExtendZodObject(nextSchema, {
          ...
          if (jsonSchema2 && typeof jsonSchema2 === "object" && jsonSchema2.type === "object") {
            ...
            const injectedKeys = [];
            if (isBackgroundEligible) {
              properties._background = chunkIHOYML2N_cjs.backgroundOverrideJsonSchema;
  ...
  getParameters = () => {
    if (chunk7Y2NL3JH_cjs.isVercelTool(this.originalTool)) {
  ...
  createExecute(tool, options, logType) {
    ...
    const mcpMeta = !chunk7Y2NL3JH_cjs.isVercelTool(tool) && "mcpMetadata" in tool ? tool.mcpMetadata : void 0;
    const execFunction = async (args, execOptions, toolSpan) => {
      try {
        let result;
        let suspendData = null;
        if (chunk7Y2NL3JH_cjs.isVercelTool(tool)) {
          result = await chunkEDFBQDWJ_cjs.executeWithContext({
            span: toolSpan,
            fn: async () => tool?.execute?.(args, execOptions)
          });
        } else {
          const wrappedMastra = options.mastra ? chunkP5FPVDFY_cjs.wrapMastra(options.mastra, { currentSpan: toolSpan }) : options.mastra;
          ...
          const baseContext = {
            ...
            runId: options.runId,
            requestContext: execOptions.requestContext ?? options.requestContext ?? new chunk64VGLWFD_cjs.RequestContext(),
            ...
            observe: execOptions.observe ?? chunkRUTNLYPF_cjs.noopObserve,
            ...
            ...chunkP5FPVDFY_cjs.createObservabilityContext({ currentSpan: toolSpan }),
          ...
          if (resumeData) {
            const resumeValidation = chunk7Y2NL3JH_cjs.validateToolInput(resumeSchema, resumeData, options.name);
          ...
          result = await chunkEDFBQDWJ_cjs.executeWithContext({ span: toolSpan, fn: async () => tool?.execute?.(args, toolContext) });
        }
        if (suspendData) {
          const suspendSchema = this.getSuspendSchema();
          const suspendValidation = chunk7Y2NL3JH_cjs.validateToolSuspendData(suspendSchema, suspendData, options.name);
        ...
        if (chunk7Y2NL3JH_cjs.isVercelTool(tool)) {
          ...
          const outputValidation = chunk7Y2NL3JH_cjs.validateToolOutput(outputSchema, result, options.name, false);
    ...
    return async (args, execOptions) => {
      ...
      const toolSpan = chunkEDFBQDWJ_cjs.getOrCreateSpan({
        ...
        input: args,
        entityType: chunkXDJV2I6J_cjs.EntityType.TOOL,
      ...
      if (fgaProvider) {
        const { getAgentToolFGAResourceId, getMCPToolFGAResourceId, getStandaloneToolFGAResourceId, requireFGA } = await import('./fga-check-IMD7W3WT.cjs');
        ...
        await requireFGA({
          ...
          permission: chunkIV2LFMTD_cjs.MastraFGAPermissions.TOOLS_EXECUTE,
          requestContext: toolRequestContext,
          context: {
            resourceId: options.resourceId
          },
      ...
      try {
        ...
        const { data, error } = chunk7Y2NL3JH_cjs.validateToolInput(parameters, args, options.name);
  ...
  build() {
    ...
    if (chunk7Y2NL3JH_cjs.isVercelTool(this.originalTool) && "needsApproval" in this.originalTool) {
    ...
    const instanceNeedsApprovalFn = chunk7Y2NL3JH_cjs.getNeedsApprovalFn(this.originalTool);
...
function ensureToolProperties(tools) {
  const toolsWithProperties = Object.keys(tools).reduce((acc, key) => {
    ...
    if (tool) {
      if (typeof tool === "function" && !(tool instanceof chunk7Y2NL3JH_cjs.Tool) && !chunk7Y2NL3JH_cjs.isVercelTool(tool)) {
...
//# sourceMappingURL=chunk-NLACGSDP.cjs.map
//# sourceMappingURL=chunk-NLACGSDP.cjs.map