...
var chunkFWVNVEOQ_cjs = require('./chunk-FWVNVEOQ.cjs');
var chunk4SCYX53T_cjs = require('./chunk-4SCYX53T.cjs');
var chunkEM3PF44Y_cjs = require('./chunk-EM3PF44Y.cjs');
...
var AIV5Adapter = class {
  ...
  static fromUIMessage(uiMsg) {
    ...
    const toolInvocationParts = parts.filter((p) => chunkFWVNVEOQ_cjs.isToolUIPart(p));
    ...
    const v2Parts = parts.map((p) => {
      if (chunkFWVNVEOQ_cjs.isToolUIPart(p)) {
        const toolName = getToolName(p);
  ...
  static toUIMessage(dbMsg) {
    ...
    if (!hasToolInvocationParts || !hasReasoningParts || !hasFileParts || !hasTextParts) {
      for (const part of v5Message.parts) {
        if (chunkFWVNVEOQ_cjs.isToolUIPart(part)) {
...
var CacheKeyGenerator = class _CacheKeyGenerator {
  ...
  static fromAIV5Parts(parts) {
    ...
    for (const part of parts) {
      ...
      if (chunkFWVNVEOQ_cjs.isToolUIPart(part) || part.type === "dynamic-tool") {
        key += part.toolCallId;
        key += part.state;
      }
      if (part.type === "reasoning") {
...
function sanitizeV5UIMessages(messages, filterIncompleteToolCalls = false) {
  ...
  const getSafeParts = (m, assistantTurnStillOpen) => m.parts.filter((p) => {
    ...
    if (!chunkFWVNVEOQ_cjs.isToolUIPart(p)) return true;
  ...
  const msgs = messages.map((m, idx) => {
    ...
    const sanitized = {
      ...
      parts: mergedParts.map((part) => {
        if (chunkFWVNVEOQ_cjs.isToolUIPart(part) && part.state === "output-available") {
          return {
            ...part,
...
function addStartStepPartsForAIV5(messages) {
  for (const message of messages) {
    ...
    for (const [index, part] of message.parts.entries()) {
      if (!chunkFWVNVEOQ_cjs.isToolUIPart(part)) continue;
      const nextPart = message.parts.at(index + 1);
      if (nextPart && nextPart.type !== `step-start` && !chunkFWVNVEOQ_cjs.isToolUIPart(nextPart)) {
        message.parts.splice(index + 1, 0, { type: "step-start" });
      }
      if (nextPart && chunkFWVNVEOQ_cjs.isToolUIPart(nextPart) && !part.providerExecuted && nextPart.providerExecuted && (nextPart.state === "output-available" || nextPart.state === "output-error")) {
        message.parts.splice(index + 1, 0, { type: "step-start" });
      }
    }
  }
...
function aiV4UIMessagesToAIV4CoreMessages(messages) {
  return chunkEM3PF44Y_cjs.convertToCoreMessages(sanitizeAIV4UIMessages(messages));
}
...
function aiV5UIMessagesToAIV5ModelMessages(messages, dbMessages, filterIncompleteToolCalls = false) {
  ...
  for (const uiMsg of preprocessed) {
    const produced = chunkFWVNVEOQ_cjs.convertToModelMessages([uiMsg]);
...
var StepContentExtractor = class _StepContentExtractor {
  ...
  static convertPartsToContent(parts, stepId, stepContentFn) {
    ...
    const modelMessages = chunkFWVNVEOQ_cjs.convertToModelMessages(sanitizeV5UIMessages(stepUiMessages));
...
var MessageList = class {
  ...
  transformToolStateDataForTranscript(data, phase) {
    ...
    const stateData = data;
    const metadata = stateData.metadata ?? stateData.providerMetadata;
    const phaseTransform = chunkVPZGHUNF_cjs.getTransformedToolPayload(metadata, "transcript", phase);
    const inputTransform = chunkVPZGHUNF_cjs.getTransformedToolPayload(metadata, "transcript", "input-available");
    const transformedArgs = phase === "approval" ? chunkVPZGHUNF_cjs.hasTransformedToolPayload(phaseTransform) ? phaseTransform.transformed : chunkVPZGHUNF_cjs.hasTransformedToolPayload(inputTransform) ? inputTransform.transformed : void 0 : chunkVPZGHUNF_cjs.hasTransformedToolPayload(inputTransform) ? inputTransform.transformed : chunkVPZGHUNF_cjs.hasTransformedToolPayload(phaseTransform) ? phaseTransform.transformed : void 0;
    const transformedSuspendPayload = phase === "suspend" && chunkVPZGHUNF_cjs.hasTransformedToolPayload(phaseTransform) ? phaseTransform.transformed : void 0;
    return {
      ...stateData,
      ...transformedArgs !== void 0 ? { args: transformedArgs } : {},
      ...transformedSuspendPayload !== void 0 ? { suspendPayload: transformedSuspendPayload } : {}
  ...
  transformMessageForTranscript(message) {
    ...
    const parts = message.content.parts.map((part) => {
      if (part.type === "tool-invocation" && part.toolInvocation) {
        const inputTransform = chunkVPZGHUNF_cjs.getTransformedToolPayload(part.providerMetadata, "transcript", "input-available");
        const outputTransform = part.toolInvocation.state === "result" ? chunkVPZGHUNF_cjs.getTransformedToolPayload(part.providerMetadata, "transcript", "output-available") ?? chunkVPZGHUNF_cjs.getTransformedToolPayload(part.providerMetadata, "transcript", "error") : part.toolInvocation.state === "output-error" ? chunkVPZGHUNF_cjs.getTransformedToolPayload(part.providerMetadata, "transcript", "error") : void 0;
        if (!inputTransform && !outputTransform) {
          return part;
        }
        changed = true;
        const transformedArgs = chunkVPZGHUNF_cjs.hasTransformedToolPayload(inputTransform) ? inputTransform.transformed : part.toolInvocation.args;
        const transformedResult = part.toolInvocation.state === "result" ? chunkVPZGHUNF_cjs.hasTransformedToolPayload(outputTransform) ? outputTransform.transformed : part.toolInvocation.result : void 0;
        const transformedErrorText = part.toolInvocation.state === "output-error" ? chunkVPZGHUNF_cjs.hasTransformedToolPayload(outputTransform) ? outputTransform.transformed : part.toolInvocation.errorText : void 0;
  ...
  updateLastCreatedAt(message) {
    ...
    const latestPartTime = Array.isArray(message.content.parts) ? message.content.parts.reduce((latest, part) => {
...
//# sourceMappingURL=chunk-DCCAHUCG.cjs.map
//# sourceMappingURL=chunk-DCCAHUCG.cjs.map