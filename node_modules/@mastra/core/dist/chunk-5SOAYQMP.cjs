...
var chunkXEHGVQJW_cjs = require('./chunk-XEHGVQJW.cjs');
var chunk7Y2NL3JH_cjs = require('./chunk-7Y2NL3JH.cjs');
var chunkOXCC7WGI_cjs = require('./chunk-OXCC7WGI.cjs');
var chunk4CVPHNP6_cjs = require('./chunk-4CVPHNP6.cjs');
...
var CompositeFilesystem = class {
  ...
  assertWritable(fs6, path9, operation) {
    if (fs6.readOnly) {
      throw new chunkXEHGVQJW_cjs.PermissionError(path9, `${operation} (filesystem is read-only)`);
    }
  }
  // ===========================================================================
...
var MastraFilesystem = class extends chunkOXCC7WGI_cjs.MastraBase {
  ...
  _onDestroy;
  constructor(options) {
    super({ name: options.name, component: chunk4CVPHNP6_cjs.RegisteredLogger.WORKSPACE });
  ...
  async ensureReady() {
    ...
    if (this.status !== "ready") {
      throw new chunkXEHGVQJW_cjs.FilesystemNotReadyError(this.id);
    }
  }
  /**
...
async function fsStat(absolutePath, userPath) {
  try {
    ...
    if (isEnoentError(error)) {
      throw new chunkXEHGVQJW_cjs.FileNotFoundError(userPath);
    }
    throw error;
  }
}
...
var LocalFilesystem = class extends MastraFilesystem {
  ...
  resolvePath(inputPath) {
    ...
    if (this._contained) {
      if (!this._isWithinAnyRoot(absolutePath)) {
        throw new chunkXEHGVQJW_cjs.PermissionError(inputPath, this._accessOperationHint(inputPath));
      }
    }
    return absolutePath;
  }
  ...
  assertWritable(operation) {
    if (this.readOnly) {
      throw new chunkXEHGVQJW_cjs.WorkspaceReadOnlyError(operation);
    }
  }
  /**
  ...
  async assertPathContained(absolutePath) {
    ...
    if (!isWithinRoot) {
      throw new chunkXEHGVQJW_cjs.PermissionError(absolutePath, "access");
    }
  }
  async readFile(inputPath, options) {
    ...
    try {
      ...
      if (stats.isDirectory()) {
        throw new chunkXEHGVQJW_cjs.IsDirectoryError(inputPath);
      }
      if (options?.encoding) {
      ...
      if (error instanceof chunkXEHGVQJW_cjs.IsDirectoryError) throw error;
      if (isEnoentError(error)) {
        throw new chunkXEHGVQJW_cjs.FileNotFoundError(inputPath);
      }
      throw error;
    }
  }
  async writeFile(inputPath, content, options) {
    ...
    if (options?.recursive === false) {
      ...
      try {
        ...
        if (!stat4.isDirectory()) {
          throw new chunkXEHGVQJW_cjs.NotDirectoryError(parentPath);
        }
      } catch (error) {
        if (error instanceof chunkXEHGVQJW_cjs.NotDirectoryError) throw error;
        if (isEnoentError(error)) {
          throw new chunkXEHGVQJW_cjs.DirectoryNotFoundError(parentPath);
        }
        throw error;
      }
    }
    ...
    if (options?.expectedMtime) {
      try {
        ...
        if (currentStat.mtime.getTime() !== options.expectedMtime.getTime()) {
          throw new chunkXEHGVQJW_cjs.StaleFileError(inputPath, options.expectedMtime, currentStat.mtime);
        }
      } catch (error) {
        if (error instanceof chunkXEHGVQJW_cjs.StaleFileError) throw error;
    ...
    try {
      ...
      if (options?.overwrite === false && isEexistError(error)) {
        throw new chunkXEHGVQJW_cjs.FileExistsError(inputPath);
      }
      throw error;
    }
  }
    ...
    try {
      ...
      if (stats.isDirectory()) {
        if (!options?.recursive) {
          throw new chunkXEHGVQJW_cjs.IsDirectoryError(src);
        }
        ...
        try {
          ...
          if (options?.overwrite === false && isEexistError(error)) {
            throw new chunkXEHGVQJW_cjs.FileExistsError(dest);
          }
          throw error;
        }
      }
    } catch (error) {
      if (error instanceof chunkXEHGVQJW_cjs.IsDirectoryError || error instanceof chunkXEHGVQJW_cjs.FileExistsError) throw error;
      if (isEnoentError(error)) {
        throw new chunkXEHGVQJW_cjs.FileNotFoundError(src);
      }
      throw error;
    }
  }
  ...
  async moveFile(src, dest, options) {
    ...
    try {
      ...
      if (error instanceof chunkXEHGVQJW_cjs.FileExistsError) throw error;
      if (isEnoentError(error)) {
        throw new chunkXEHGVQJW_cjs.FileNotFoundError(src);
      }
      throw error;
  ...
  async rmdir(inputPath, options) {
    ...
    try {
      const stats = await fs2__namespace.stat(absolutePath);
      if (!stats.isDirectory()) {
        throw new chunkXEHGVQJW_cjs.NotDirectoryError(inputPath);
      }
      if (options?.recursive) {
        ...
        if (entries.length > 0) {
          throw new chunkXEHGVQJW_cjs.DirectoryNotEmptyError(inputPath);
        }
        await fs2__namespace.rmdir(absolutePath);
      }
    } catch (error) {
      if (error instanceof chunkXEHGVQJW_cjs.NotDirectoryError || error instanceof chunkXEHGVQJW_cjs.DirectoryNotEmptyError) {
        throw error;
      }
      if (isEnoentError(error)) {
        if (!options?.force) {
          throw new chunkXEHGVQJW_cjs.DirectoryNotFoundError(inputPath);
        }
      } else {
        throw error;
      }
...
function isLSPAvailable() {
  ...
  try {
    const req = module$1.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('chunk-5SOAYQMP.cjs', document.baseURI).href)));
...
var DEFAULT_MAX_RETAINED_PROCESS_OUTPUT_BYTES = 1024 * 1024;
var RETAINED_OUTPUT_COMPACT_CHUNK_THRESHOLD = 128;
...
function getPreviousCodePointStart(value, end) {
  let start = end - 1;
  const codeUnit = value.charCodeAt(start);
  if (codeUnit >= 56320 && codeUnit <= 57343 && start > 0) {
    const previousCodeUnit = value.charCodeAt(start - 1);
    if (previousCodeUnit >= 55296 && previousCodeUnit <= 56319) {
      start -= 1;
    }
  }
  return start;
...
var SandboxProcessManager = class {
  ...
  constructor({ env = {} } = {}) {
    ...
    this.spawn = async (...args) => {
      if (args[1]?.maxRetainedBytes !== void 0) {
...
var MastraSandbox = class extends chunkOXCC7WGI_cjs.MastraBase {
  /** Process manager */
  processes;
...
function parseSkillSnapshotFromFiles(files) {
  ...
  const skillMdContent = typeof skillMdFile.content === "string" ? skillMdFile.content : skillMdFile.content.toString("utf-8");
  const parsed = matter__default.default(skillMdContent);
  const frontmatter = parsed.data;
...
var ENV_FIELD_NAMES = /* @__PURE__ */ new Set(["env", "environment", "process_env"]);
var SECRET_FIELD_PATTERN = /(^|[_-])(api[_-]?key|key|token|secret|password|passwd|pwd|credential|credentials|auth|authorization|cookie|session)([_-]|$)/i;
...
function createSkillTool(skills) {
  const tool = chunk7Y2NL3JH_cjs.createTool({
    id: "skill",
...
var Workspace = class {
  ...
  constructor(config) {
    ...
    if (config.mounts && Object.keys(config.mounts).length > 0) {
      if (config.filesystem) {
        throw new chunkXEHGVQJW_cjs.WorkspaceError('Cannot use both "filesystem" and "mounts"', "INVALID_CONFIG");
      }
      ...
      if (/^class\s/.test(Function.prototype.toString.call(config.filesystem))) {
        throw new chunkXEHGVQJW_cjs.WorkspaceError(
    ...
    if (config.vectorStore && !config.embedder) {
      throw new chunkXEHGVQJW_cjs.WorkspaceError("vectorStore requires an embedder", "INVALID_SEARCH_CONFIG");
    }
    ...
    if (!this._fs && !this._filesystemResolver && !this._sandbox && !this.hasSkillsConfig()) {
      throw new chunkXEHGVQJW_cjs.WorkspaceError("Workspace requires at least a filesystem, sandbox, or skills", "NO_PROVIDERS");
    }
  }
  generateId() {
  ...
  async index(path9, content, options) {
    if (!this._searchEngine) {
      throw new chunkXEHGVQJW_cjs.SearchNotAvailableError();
    }
    this.lastAccessedAt = /* @__PURE__ */ new Date();
    const doc = {
      id: path9,
  ...
  async getInfo(options) {
    ...
    const filesystem = this._fs ?? (this._filesystemResolver ? await this.resolveFilesystem({ requestContext: options?.requestContext ?? new chunk64VGLWFD_cjs.RequestContext() }) : void 0);
    if (filesystem) {
...
function requireWorkspace(context) {
  if (!context?.workspace) {
    throw new chunkXEHGVQJW_cjs.WorkspaceNotAvailableError();
  }
...
function requireFilesystem(context) {
  ...
  if (!workspace.filesystem) {
    throw new chunkXEHGVQJW_cjs.FilesystemNotAvailableError();
  }
...
function requireSandbox(context) {
  ...
  if (!workspace.sandbox) {
    throw new chunkXEHGVQJW_cjs.SandboxNotAvailableError();
  }
...
var astEditTool = chunk7Y2NL3JH_cjs.createTool({
  ...
  execute: async ({ path: path9, pattern, replacement, transform, targetName, newName, importSpec }, context) => {
    ...
    try {
      if (filesystem.readOnly) {
        throw new chunkXEHGVQJW_cjs.WorkspaceReadOnlyError("ast_edit");
      }
      ...
      try {
        ...
        if (error instanceof chunkXEHGVQJW_cjs.FileNotFoundError) {
          span.end({ success: false });
...
var deleteFileTool = chunk7Y2NL3JH_cjs.createTool({
  ...
  execute: async ({ path: path9, recursive }, context) => {
    ...
    try {
      if (filesystem.readOnly) {
        throw new chunkXEHGVQJW_cjs.WorkspaceReadOnlyError("delete");
      }
...
var editFileTool = chunk7Y2NL3JH_cjs.createTool({
...
- Use replace_all only when intentionally replacing all occurrences.`,
  ...
  execute: async ({ path: path9, old_string, new_string, replace_all }, context) => {
    ...
    try {
      if (filesystem.readOnly) {
        throw new chunkXEHGVQJW_cjs.WorkspaceReadOnlyError("edit_file");
      }
      ...
      let output = `Replaced ${result.replacements} occurrence${result.replacements !== 1 ? "s" : ""} in ${path9}${lineRanges}`;
...
async function applyTokenLimit(output, limit = DEFAULT_MAX_OUTPUT_TOKENS, from = "start") {
  if (!output) return output;
  const totalTokens = tokenx.estimateTokenCount(output);
  if (totalTokens <= limit) return output;
  const kept = from === "start" ? tokenx.sliceByTokens(output, -limit) : tokenx.sliceByTokens(output, 0, limit);
...
async function applyTokenLimitSandwich(output, limit = DEFAULT_MAX_OUTPUT_TOKENS, headRatio = 0.1) {
  ...
  const head = headBudget > 0 ? tokenx.sliceByTokens(output, 0, headBudget) : "";
  const tail = tailBudget > 0 ? tokenx.sliceByTokens(output, -tailBudget) : "";
...
async function executeCommand(input, context) {
  ...
  if (background) {
    if (!sandbox.processes) {
      const err = new chunkXEHGVQJW_cjs.SandboxFeatureNotSupportedError("processes");
      span.error(err);
      throw err;
    }
  ...
  if (!sandbox.executeCommand) {
    const err = new chunkXEHGVQJW_cjs.SandboxFeatureNotSupportedError("executeCommand");
    span.error(err);
    throw err;
  }
...
var executeCommandTool = chunk7Y2NL3JH_cjs.createTool({
...
var executeCommandWithBackgroundTool = chunk7Y2NL3JH_cjs.createTool({
  id: WORKSPACE_TOOLS.SANDBOX.EXECUTE_COMMAND,
  description: `${baseDescription}
...
var fileStatTool = chunk7Y2NL3JH_cjs.createTool({
...
var getProcessOutputTool = chunk7Y2NL3JH_cjs.createTool({
...
Use this after starting a background command with execute_command (background: true) to check if the process is still running and read its output.`,
  ...
  execute: async ({ pid, tail, wait: shouldWait }, context) => {
    ...
    try {
      if (!sandbox.processes) {
        throw new chunkXEHGVQJW_cjs.SandboxFeatureNotSupportedError("processes");
      }
...
var grepTool = chunk7Y2NL3JH_cjs.createTool({
...
var indexContentTool = chunk7Y2NL3JH_cjs.createTool({
...
var killProcessTool = chunk7Y2NL3JH_cjs.createTool({
...
var listFilesTool = chunk7Y2NL3JH_cjs.createTool({
...
var lspInspectTool = chunk7Y2NL3JH_cjs.createTool({
...
var mkdirTool = chunk7Y2NL3JH_cjs.createTool({
  ...
  execute: async ({ path: path9, recursive }, context) => {
    ...
    try {
      if (filesystem.readOnly) {
        throw new chunkXEHGVQJW_cjs.WorkspaceReadOnlyError("mkdir");
      }
...
var DEFAULT_MEDIA_TYPES = ["image/png", "image/jpeg", "image/webp", "application/pdf"];
var DEFAULT_MAX_MEDIA_BYTES = 10 * 1024 * 1024;
var TEXT_APPLICATION_TYPES = /* @__PURE__ */ new Set([
  "application/json",
...
var MEDIA_TYPE_PATTERN = /^(?:\*|\*\/\*|[a-z0-9!#$&^_.+-]+\/(?:\*|[a-z0-9!#$&^_.+-]+))$/i;
...
function buildMediaTypeCheck(config) {
  ...
  if (typeof config === "function") {
    return (mimeType) => mimeType ? config(mimeType) : false;
  }
...
var readFileTool = chunk7Y2NL3JH_cjs.createTool({
  ...
  inputSchema: v4.z.object({
    ...
    offset: v4.z.number().int().min(1).optional().describe(
    ...
    limit: v4.z.number().int().min(1).optional().describe(
    ...
    showLineNumbers: v4.z.boolean().optional().default(true).describe(
    ...
    encoding: v4.z.enum(["utf-8", "utf8", "base64", "hex", "binary"]).optional().describe(
  ...
  execute: async ({ path: path9, encoding, offset, limit, showLineNumbers }, context) => {
    ...
    try {
      ...
      if (!encoding && shouldReturnAsMedia(stat4.mimeType)) {
        const maxMediaBytes = readFileConfig?.maxMediaBytes ?? DEFAULT_MAX_MEDIA_BYTES;
        if (stat4.size > maxMediaBytes) {
          span.end({ success: true }, { bytesTransferred: 0 });
          return `${stat4.path} (${stat4.size} bytes, ${stat4.mimeType}) \u2014 exceeds maxMediaBytes (${maxMediaBytes}). Returning metadata only; configure \`maxMediaBytes\` on the read_file tool to raise this cap.`;
        }
        const base64 = await filesystem.readFile(path9, { encoding: "base64" });
        const header2 = `${stat4.path} (${stat4.size} bytes, ${stat4.mimeType})`;
        span.end({ success: true }, { bytesTransferred: stat4.size });
        return {
          __workspaceMedia: true,
          text: header2,
          mediaType: stat4.mimeType,
          data: base64
        };
      }
      if (!encoding && !isTextLikeMimeType(stat4.mimeType)) {
        span.end({ success: true }, { bytesTransferred: 0 });
        return `${stat4.path} (${stat4.size} bytes, ${stat4.mimeType ?? "unknown"}) \u2014 binary file not readable as text. Pass an explicit \`encoding\` (e.g. \`base64\`) to read the raw bytes, or configure \`mediaTypes\` on the read_file tool to surface it as a media part.`;
      }
...
var searchTool = chunk7Y2NL3JH_cjs.createTool({
...
var writeFileTool = chunk7Y2NL3JH_cjs.createTool({
  ...
  execute: async ({ path: path9, content, overwrite }, context) => {
    ...
    try {
      if (filesystem.readOnly) {
        throw new chunkXEHGVQJW_cjs.WorkspaceReadOnlyError("write_file");
      }
...
async function resolveEffectiveWorkspace(workspace, context) {
  if (!workspace.filesystem && hasFilesystemConfig(workspace)) {
    const requestContext = context?.requestContext ?? new chunk64VGLWFD_cjs.RequestContext();
          ...
          if (config.requireReadBeforeWrite !== void 0) {
            ...
            if (shouldRequireRead) {
              ...
              if (check.needsReRead) {
                throw new chunkXEHGVQJW_cjs.FileReadRequiredError(input.path, check.reason);
              }
            }
          }
        } catch (error) {
...
exports.formatSkillActivation = formatSkillActivation;
...
//# sourceMappingURL=chunk-5SOAYQMP.cjs.map
//# sourceMappingURL=chunk-5SOAYQMP.cjs.map